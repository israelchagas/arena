...
import {
  ArrowLeft, User, Dumbbell, Phone, CheckCircle,
  AlertCircle, HelpCircle, ChevronRight, Shield,
} from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-black text-gray-900 mb-2">Atleta inscrito!</h2>
          <p className="text-gray-500 text-sm mb-1">
            A inscrição foi salva com sucesso.
          </p>
          <p className="text-gray-400 text-xs mb-8">
            Total de atletas inscritos pela sua associação:{" "}
            <strong className="text-gray-700">{totalInscritos}</strong>
          </p>
          <div className="space-y-3">
            <button
              onClick={inscreverOutro}
            ...
            <button
              ...
              className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 rounded-xl text-sm transition-all"
            >
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          <button
            onClick={() => navigate("/professor")}
            className="w-9 h-9 rounded-xl hover:bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-700 transition-colors"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <p className="font-bold text-gray-900 text-sm leading-none">Inscrever Atleta</p>
            <p className="text-gray-400 text-xs mt-0.5">
              {(professor?.associacoes as any)?.nome ?? ""} · {totalInscritos} inscrito{totalInscritos !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="bg-blue-600 rounded-xl px-5 py-4 mb-6 flex items-center justify-between">
          <div>
            <p className="text-blue-200 text-xs font-medium uppercase tracking-wide">Associação</p>
          ...
          <div className="text-right">
            ...
            <p className="text-white font-semibold text-sm">{professor?.nome}</p>
          </div>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">

          {/* ── Seção 1: Dados pessoais do atleta ───────────────── */}
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            <div>
              <Label>CPF do atleta <span className="text-gray-400 font-normal">(opcional — se não possuir, informe apenas o do responsável)</span></Label>
              <input
                {...register("atleta_cpf")}
                type="text"
                inputMode="numeric"
            ...
            <div className="mb-4">
              ...
              {!sexoSelecionado ? (
                ...
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                  ...
                  <p className="text-gray-500 text-xs">Nenhuma categoria cadastrada para este sexo. Contate o organizador.</p>
                </div>
              ) : (
