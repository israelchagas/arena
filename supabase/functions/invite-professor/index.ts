...
function emailHtml(nome: string, email: string, password: string, loginUrl: string): string {
  return `<!DOCTYPE html>
<html>
  ...
  <tr>
    <td style="padding:36px 40px;">
      ...
      <p style="font-size:15px;color:#64748b;line-height:1.7;margin:0 0 28px;">
        Seu cadastro foi <strong style="color:#16a34a;">aprovado</strong>! Use as credenciais abaixo para acessar o sistema e gerenciar as inscrições dos seus atletas.
      </p>
...
async function sendResend(apiKey: string, to: string, nome: string, email: string, password: string, appUrl: string) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const appUrl      = Deno.env.get("APP_URL") ?? "http://localhost:3010";
    ...
    if (action === "approve") {
      ...
      const { data: profileData, error: profileErr } = await arena
        .from("profiles")
        .insert({ user_id: userId, role: "professor", nome: prof.nome })
        .select("id")
        .single();
      ...
      const { error: updateErr } = await arena
        .from("professores")
        .update({ profile_id: profileData.id, status: "ativo", invited_at: new Date().toISOString() })
        .eq("id", professor_id);
    ...
    if (action === "resend") {
      ...
      try {
        ...
        console.error("[invite-professor] Email failed on resend:", e);
      }
    ...
    const { data: profileData, error: profileErr } = await arena
      .from("profiles")
      .insert({ user_id: userId, role: "professor", nome })
      .select("id")
      .single();
